# 점진적인 개선 (Successive refinement)

먼저 잘 만들어진 코드를 보여준 뒤, 어떻게 그런 코드를 만들었는지 점진적으로 개선하는 과정을 보여준다.
...
```
사용법이 간단하다. 매개변수 두 개로 Args 클래스의 클래스의 인스턴스를 만들었다.
첫번째 parameter는 형식 또는 스키마를 지정하는 "l,p#,d*". 첫 번째 -l은 boolean, 두 번째 -p는 Integer, 세 번째 -d는 String.
두번째 parameter는 main으로 넘어온 명령행 인수 배열.
...
```
public method를 아래에 배치해서라도 읽는 흐름을 중요시했다.
```

ArgumentMarshaler Interface를 먼저 봐야된다. 
...
나머지 DoubleArgumentMarshaler와 StringArrayArgumentMarshaler는 생략한다. 
...
```
TODO를 이용해 개요를 작성하고, 하나씩 구현해나간다. 구현이 끝나면 돌아보며 리팩토링을 한다.
다음에 코드를 볼일이 생기면 뭔가 부족한 부분이 보이고, 다시 리팩토링을 한다.
```

## Args: 1차 초안
목록 14-8은 내가 맨 처음 짰던 Args 클래스다. 코드는 '돌아가지만' 엉망이다.

##### Boolean, String, Integer가 모두 들어간 초기 버전
```java
...
그래서 나는 **기능을 더 이상 추가하지 않기로 결정하고 리팩터링을 시작**했다. String, Integer 유형을 추가한 경험에서 나는 **새 인수 유형을 추가하려면 주요 지점 세 곳에도 코드를 추가해야 한다는 사실을 이미 깨달았다.**
...
```
나는 리팩토링을 할때마다 테스트가 깨져서 골치가 아팠는데, 밥 아저씨는 테스트를 믿고 리팩토링을 했다. 어떤 차이가 있을까?
```
...
// ArgumentMarshaler를 리턴하기 때문에 getBoolean으로 값을 가져옴
public boolean getBoolean(char arg) {
	return falseIfNull(booleanArgs.get(arg).getBoolean());
}

// falseIfNull이 의미가 없기 때문에 제거
public boolean getBoolean(char arg) {
	return booleanArgs.get(arg).getBoolean();
}

// null을 체크하기 위해 두줄로 분리, 이름은 am으로 간단하게 사용
public boolean getBoolean(char arg) {
...
```
Ownership
언젠가부터 Company-wide Perspective로 변경되었는데, leadership principles에는 원래 Ownership이 있었다. Amazon은 여전히 있다.
개발자에게는 Ownership보다 Craftmanship이 필요하지 않을까?
```